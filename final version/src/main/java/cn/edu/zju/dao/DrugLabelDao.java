...
public class DrugLabelDao extends BaseDao {
    ...
    public List<DrugLabel> getDrugLabel() {
        List<DrugLabel> TotalLabels = new ArrayList<>();
        DBUtils.execSQL(connection -> {
            try {
                PreparedStatement preparedStatement = connection.prepareStatement("Select id,name,drug_id,alternate_drug_available,summary_markdown from drug_label");
